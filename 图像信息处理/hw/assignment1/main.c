...
int main(int argc, char* argv[])
{
	...
	fpbmp = openfile("1.bmp");  //open the source file 
	bmpDataPart(fpbmp);
	
	fpout = writefile("2.bmp");  //output the file
	AddHeader2File(fpbmp, fpout);

	/*operations for the picture*/
	RGB2YUV();  
	YUV2Gray();  //include rearrange
	//ChangeTheLuminance(); 
	YUV2RGB();
...
{
	FILE *fpout;
	fpout = fopen("2.bmp", "wb+");  //write the grayscale bmp
	//fpout = fopen("3.bmp", "wb+");  //write the color bmp
	if (fpout == NULL) {
