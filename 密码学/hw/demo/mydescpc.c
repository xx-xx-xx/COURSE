...
static char fp[64] = {
    /* [%] source Bit1-> target Bit58, Bit2->Bit50, ..., Bit64->Bit7 */
...
static long (*sbox_output_perm_table)[64]; /*[%] 8 * 64 * long:
                                                 8 = 8 boxes
                                                64 = 6-bit input
                                              long = scattered 32-bit output produced
...
int des_init(int mode)
{
    ...
    if ((sbox_output_perm_table = (long(*)[64])malloc(sizeof(long) * 8 * 64)) == NULL)
    {
        return 0;
    }
...
static perm_init(char perm[16][16][8], char p[64]) /* 64-bit, either init or final */
{
    register int b, j, k;
    int i, m;
...
int des_cbc_encrypt(char p[], char c[], int n, char iv[])
{
    int i, j;
    unsigned char t[8];
    if (p == NULL || c == NULL || iv == NULL)
        return 0;
    for (i = 0; i < n / 8; i++)
    {
        for (j = 0; j < 8; j++)
...
int des_cfb_encrypt(char p[], char c[], int bits, int n, char iv[])
{
    int i, j, bytes = (bits + 7) / 8, count = 0;
    ...
    while (n >= bytes)
    {
        ...
        else
        {
            for (i = 0; i < 8; i++)
            {
                t[i] = t[i + bits / 8] << bits % 8 | t[i + bits / 8 + 1] >> (8 - bits % 8);
            }
        }
        memcpy(sv, t, 8);
        memcpy(iv, t, 8);
...
{
    char mykey[] = "ABCD1234";
    char iv[] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF};
    char old_iv[] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF};
    char p[] = "abcdefgh12345678xyz";
    unsigned char c[100];
    int i, n;
    des_init(0);
    des_set_key(mykey);
